...
Hardware we used
2× Arduino Nano 33 BLE Sense Rev1 (Patient + CAretaker) VL53L0X (infrared proximity sensor)

Given Board:
MAX30102 (SpO₂ + heart rate sensor, external breakout) HTS221 (built into the Nano — handles temp & humidity) 5 push buttons + resistors for the voltage divider network

Wiring the MAX30102 and VL53L0X — both connect over I²C: VIN → 3.3V, GND → GND, SDA → A4, SCL → A5
...
ArduinoBLE Arduino_HTS221 SparkFun MAX3010x Pulse and Proximity Sensor Library Pololu VL53L0X
...
Temperature: alert if > 38°C (fever) or < 35°C (hypothermia) Humidity: alert if < 30% or > 70% SpO₂: alert if < 95% BPM: alert if < 50 or > 120 Motion: alert on sustained sporadic movement
